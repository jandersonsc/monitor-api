...
export const customerSignUpValidator = vine.compile(
  vine.object({
    ...
    document: vine.string().minLength(11).maxLength(14).unique({ table: 'customers', column: 'document' }),
    email: vine.string().email().unique({ table: 'customers', column: 'email' }),
    password: vine.string().minLength(8).maxLength(255),
    confirmPassword: vine.string().minLength(8).maxLength(255).confirmed({ confirmationField: 'password' }),
